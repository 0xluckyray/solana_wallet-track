...
# def get_average_buy_price(wallet_address):
#     url = f"https://feed-api.cielo.finance/api/v1/{wallet_address}/pnl/tokens"
#     headers = {
...
#         if data['status'] == 'ok':
#             buy_prices = [tx['average_buy_price'] for tx in data['data']['items']]
#             if buy_prices:
#                 average_buy_price = sum(buy_prices) / len(buy_prices)
#                 return average_buy_price
#             else:
#                 return "No buy prices found."
#         else:
...
def save_track_wallets():
  ...
  MONGODB_URL=os.getenv("MONGODB_URL")  # Load Scraper API key from environment variables
  ...
  print(f"pair_addresses", pair_addresses)  # Print the extracted pair addresses
  ...
  for pair_address in pair_addresses:
      ...
      for address in sorted_wallets:
        ...
        if total_pnl < -10000:
          print(f"address {address}: total loss amount of any coin({total_pnl}) is over than 10000 (requirements 3 - B)")
